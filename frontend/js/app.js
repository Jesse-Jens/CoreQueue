...
function loadServerStorage() {
  try {
    ...
    if (xhr.status === 200 && xhr.responseText) {
      ...
      ['accounts','organisations','tickets','nextTicketId','nextOrgId','adminPassword'].forEach(key => {
        if (key in data) {
          const val = data[key];
          if (val !== undefined) {
...
function syncServerStorage() {
  try {
    const keys = ['accounts','organisations','tickets','nextTicketId','nextOrgId','adminPassword'];
    const data = {};
    keys.forEach(key => {
    ...
    }).catch(err => console.error('Failed to sync storage', err));
  } catch (e) {
...
function login(email, password) {
  ...
  if (!account) {
    ...
    if (linked.length > 0) {
      ...
      if (!linked[0].password || linked[0].password === pwd) {
        account = { email: normalEmail, password: pass, name: linked[0].name, organisations: linked.map(l => ({ id: l.orgId, role: l.role })) };
  ...
  if (account && account.password === pwd) {
    const globalRole = account.organisations && account.organisations.some(o => o.role === 'controller') ? 'technician' : 'customer';
