function login(email, password) {
  ...
  if (email === 'admin@corequeue.local' && password === adminPassword) {
...
function upsertAccount(email, password, name, orgId, role) {
  ...
  if (!acc) {
    acc = { email, password: password || generatePassword(), name: name || '', organisations: [] };
    accounts.push(acc);
  } else {
  ...
  updateContactPasswords(email, acc.password);
  return acc.password;
}
