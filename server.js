...
app.post('/api/cloudflare', (req, res) => {
  ...
  if (typeof body.token === 'string') {
    try { fs.writeFileSync(CF_TOKEN_FILE, body.token); } catch {}
  }
  if (body.enable === true) {
    try { execSync('systemctl enable --now corequeue-cloudflared.service'); } catch {}
  } else if (body.enable === false) {
    try { execSync('systemctl disable --now corequeue-cloudflared.service'); } catch {}
