...
app.get('/api/mail/fetch', async (req, res) => {
  ...
  try {
    ...
    const msgRes = await fetch(`https://graph.microsoft.com/v1.0/users/${encodeURIComponent(cfg.address)}/mailFolders('Inbox')/messages?$top=10&$select=id,subject,from,body,hasAttachments`, {
    ...
    for (const m of messages) {
      ...
      if (m.hasAttachments) {
        const attRes = await fetch(`https://graph.microsoft.com/v1.0/users/${encodeURIComponent(cfg.address)}/messages/${m.id}/attachments`, {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    res.status(500).json({ error: 'mail_fetch_failed' });
  }
});
