...
async function fetchIncomingMail() {
  ...
  try {
    ...
    const store = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
    const since = encodeURIComponent(store.lastMailSync || new Date().toISOString());
    const msgRes = await fetch(
      `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(cfg.address)}/mailFolders('Inbox')/messages?$top=25&$orderby=receivedDateTime%20asc&$filter=receivedDateTime%20gt%20${since}&$select=id,subject,from,body,hasAttachments,receivedDateTime`,
...
app.post('/api/mail/send', async (req, res) => {
  ...
  try {
    ...
    const sendRes = await fetch(`https://graph.microsoft.com/v1.0/users/${encodeURIComponent(cfg.address)}/sendMail`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    ...
    if (!sendRes.ok) {
      ...
      console.error('mail send failed', sendRes.status, text);
      return res.status(500).json({ error: 'mail_send_failed', details: text });
    }
...
app.post('/api/mail/test', async (req, res) => {
  ...
  try {
    ...
    if (!sendRes.ok) {
      const text = await sendRes.text();
      console.error('mail test send failed', sendRes.status, text);
      return res.status(500).json({ error: 'mail_test_failed', details: text });
    }
