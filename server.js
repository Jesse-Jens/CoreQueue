...
async function fetchIncomingMail() {
  ...
  try {
    ...
    const msgRes = await fetch(
      `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(cfg.address)}/mailFolders('Inbox')/messages?$top=10&$filter=isRead%20eq%20false&$select=id,subject,from,body,hasAttachments`,
...
app.post('/api/mail/test', async (req, res) => {
  ...
  try {
    ...
    if (!sendRes.ok) {
      ...
      return res.status(500).json({ error: 'mail_test_failed' });
    }
