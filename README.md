...
These values may also be supplied via environment variables (for example in an `.env` file):
...
Environment variables act only as fallbacks. If values are saved in `data/mail.json` via the Settings page, those values take precedence, so legacy environment variables won't override the current configuration.

### Required Azure app permissions

Create an app registration in the Microsoft Entra admin center and grant it the following **application** permissions on Microsoft Graph, then grant admin consent:
...
If the test returns `mail_access_denied` (often surfaced as “Access is denied. Check credentials and try again.”), the Azure app lacks permission to access the mailbox. Ensure the app has the **Mail.ReadWrite** and **Mail.Send** application permissions with admin consent and, when using an application access policy, that the shared mailbox is included in the allowed list.
