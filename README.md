...
If the test returns `mail_access_denied`, the Azure app lacks permission to access the mailbox. Ensure the app has the **Mail.ReadWrite** and **Mail.Send** application permissions with admin consent and, when using an application access policy, that the shared mailbox is included in the allowed list.
