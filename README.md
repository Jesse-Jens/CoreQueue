...
The server polls the mailbox every minute and also whenever a technician opens the dashboard, ensuring new messages are captured even if no one is online.

### Testing the connection

Use the **Send Test Mail** button in Settings to verify connectivity. The server attempts to send a test message from the configured mailbox back to itself and then fetches incoming messages. The response reports how many messages were fetched so you can confirm both outbound and inbound access.
